            ...
            if (class_exists('SECURE_INPUT_DATA_AVAILABLE')) {
               ...
               while ($row = $result->fetch_assoc()) {
                  ...
                  if ($total_amount > $total_balance) {
                     echo '<script type="text/javascript">alert("You do not have enough balance to do this transfer. send");
                </script>';
                  } else if ($total_amount <= $total_balance) {


                     // $sql2 = "UPDATE accounts 
                     //  SET amounts_transferred = CONCAT(amounts_transferred,  '$total_amount'), 
                     //  total_balance = CONCAT(total_balance,  '-$total_amount') 
